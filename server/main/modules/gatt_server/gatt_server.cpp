...
#define GATTS_TAG "LEDSYNC_SERVER"
...
uint8_t state_A = 0;
#define GATTS_SERVICE_UUID_TEST_A 0xDEAD
#define GATTS_CHAR_UUID_TEST_A 0xDE01
#define GATTS_DESCR_UUID_TEST_A 0xDE11
#define GATTS_NUM_HANDLE_TEST_A 4
#define MQTT_TOPIC_A "ledsync/milestone02/A/state"

uint8_t state_B = 0;
#define GATTS_SERVICE_UUID_TEST_B 0xBEEF
#define GATTS_CHAR_UUID_TEST_B 0xBE01
#define GATTS_DESCR_UUID_TEST_B 0xBE11
#define GATTS_NUM_HANDLE_TEST_B 4
#define MQTT_TOPIC_B "ledsync/milestone02/B/state"

#define TEST_DEVICE_NAME "LEDSYNC_SERVER"
...
static void gap_event_handler(esp_gap_ble_cb_event_t event,
  ...
  switch (event) {
    ...
    // When set packet length complete, the event comes
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event,
  ...
  switch (event) {
    ...
    case ESP_GATTS_READ_EVT: {
        ...
        rsp.attr_value.value[0] = state_A;
...
static void gatts_profile_b_event_handler(esp_gatts_cb_event_t event,
  ...
  switch (event) {
    ...
    case ESP_GATTS_READ_EVT: {
        ...
        rsp.attr_value.handle = param->read.handle;
        rsp.attr_value.len = 1;
        rsp.attr_value.value[0] = state_B;
        esp_ble_gatts_send_response(gatts_if, param->read.conn_id,
                                    param->read.trans_id, ESP_GATT_OK, &rsp);
        break;
    }
...
gpio_num_t GPIO_INTR_A = static_cast<gpio_num_t>(16);
gpio_num_t GPIO_INTR_B = static_cast<gpio_num_t>(5);
#define GPIO_TAG "GPIO_ISR"

static QueueHandle_t gpio_queue = NULL;

static volatile int64_t last_interrupt_time = 0;

#define DEBOUNCE_THRESHOLD_US 500000

static void IRAM_ATTR gpio_isr_change_state_handler(void *arg) {
...
static void gpio_isr_notify_change_state(void *arg) {
  int io_num;

  while (1) {
    if (xQueueReceive(gpio_queue, &io_num, portMAX_DELAY)) {
      ESP_LOGW(GPIO_TAG, "Interrupt on GPIO[%d]", io_num);
      ...
      if (io_num == GPIO_INTR_A) {
        profile = PROFILE_A_APP_ID;
        ...
        value = state_A;
        topic = MQTT_TOPIC_A;
      } else if (io_num == GPIO_INTR_B) {
        profile = PROFILE_B_APP_ID;
      ...
      if (profile == -1) {
        ESP_LOGE(GPIO_TAG, "Unknown GPIO in interrupt handler: %d", io_num);
        return;
      }

      esp_ble_gatts_send_indicate(
          gl_profile_tab[profile].gatts_if, gl_profile_tab[profile].conn_id,
          gl_profile_tab[profile].char_handle, sizeof(value), &value, false);

      char buffer[sizeof(uint8_t) * 8 + 1];
      snprintf(buffer, sizeof(uint8_t) * 8 + 1, "%d", value);

      // TODO send data with new mqtt module
      // mqtt_send_data(topic, buffer);
    }
  }
}

extern "C" void start_gatt_server(void) {
  esp_err_t ret;
  ...
  if (gpio_queue == NULL) {
    ESP_LOGE("GPIO", "Failed to create GPIO event queue");
    return;
  }

  gpio_install_isr_service(0);

  gpio_isr_handler_add(GPIO_INTR_A, gpio_isr_change_state_handler,
                       (void *)GPIO_INTR_A);
  gpio_isr_handler_add(GPIO_INTR_B, gpio_isr_change_state_handler,
                       (void *)GPIO_INTR_B);

  xTaskCreate(gpio_isr_notify_change_state, "gpio_isr_notify_change_state",
              2048, NULL, 10, NULL);
