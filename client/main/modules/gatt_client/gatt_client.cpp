...
std::unordered_map<uint16_t, uint16_t> handle_to_char;

void GattClient::set_services(
    const std::map<uint16_t, std::vector<uint16_t>> &service_data) {
  for (const auto &service_datum : service_data) {
    GattClient::Service service = {
        .uuid = {.len = ESP_UUID_LEN_16,
                 .uuid = {.uuid16 = service_datum.first}},
        .handle = 0};
...
void GattClient::gattc_profile_event_handler(esp_gattc_cb_event_t event,
    ...
    if (GattClient::get_server) {
      ...
      if (count > 0) {
        esp_gattc_char_elem_t *char_elem_result =
            (esp_gattc_char_elem_t *)malloc(sizeof(esp_gattc_char_elem_t) *
                                            count);
        if (!char_elem_result) {
          ...
          for (const auto &service : GattClient::services) {
            for (const auto &characteristic : service.characteristics) {
              ...
              if (status != ESP_GATT_OK) {
                ESP_LOGE(GATTC_TAG,
                         "esp_ble_gattc_get_char_by_uuid error %x %x",
                         char_elem_result->uuid.uuid.uuid16,
                         characteristic.uuid.uuid.uuid16);
              }
              ...
              if (count > 0 && (char_elem_result[0].properties &
                ...
                handle_to_char[char_elem_result[0].char_handle] =
                    characteristic.uuid.uuid.uuid16;
              }
            }
          }
    ...
    if (p_data->reg_for_notify.status != ESP_GATT_OK) {
      ...
      if (count > 0) {
        esp_gattc_descr_elem_t *descr_elem_result =
            new esp_gattc_descr_elem_t[count];
        if (!descr_elem_result) {
          ...
          ret_status = esp_ble_gattc_get_descr_by_char_handle(
              ...
              get_characteristic_by_handle(p_data->reg_for_notify.handle),
  ...
  case ESP_GATTC_NOTIFY_EVT: {
    ...
    uint16_t char_uuid = handle_to_char[handle];
    // ESP_LOGI(GATTC_TAG, "Notify handle is %x, uuid %x", handle, char_uuid);

    if (on_notify) {
      on_notify(char_uuid, p_data->notify.value);
    }
...
void GattClient::esp_gap_cb(esp_gap_ble_cb_event_t event,
    ...
    case ESP_GAP_SEARCH_INQ_RES_EVT:
      ...
      if (adv_name != NULL) {
        if (strlen(GattClient::remote_device_name) == adv_name_len &&
            strncmp((char *)adv_name, GattClient::remote_device_name,
                    adv_name_len) == 0) {
...
void GattClient::esp_gattc_cb(esp_gattc_cb_event_t event,
                              esp_gatt_if_t gattc_if,
                              esp_ble_gattc_cb_param_t *param) {
